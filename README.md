...
每次成功更新都展示总评分前5只股票，同时保留60分严格候选线；低于60分的股票只用于榜单比较，不进入候选池。B尾盘动能模块在14:20前不参与得分。页面中的盘面分析文字由结构化行情和评分结果按规则模板生成，不调用大模型。全市场行情覆盖率达到90%即通过完整性检查；个别分页失败会记录在数据源诊断中，覆盖率低于90%时停止生成方向判断，避免把过度残缺的行情写成当日结论。

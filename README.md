...
当前默认运行5个交易日的影子验证：每个节点对评分前5的实时价、日K收盘价和分钟线末价进行交叉核对，结果写入快照诊断和研究历史，但不参与评分。集合竞价窗口会额外检查竞价接口。完成5个交易日且覆盖率、时间戳和价格差异符合阈值后，再将配置中的 `data_sources.tdxaidata.mode` 从 `shadow` 调整为主源模式。
